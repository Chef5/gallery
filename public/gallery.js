  ...
  function initGallery() {
    ...
    function createTagFilter(tags) {
      ...
      const filteredTags = tags
        .filter((tag) => tag !== "all" && tag !== "0_preview")
        .sort();
    ...
    function filterImages(tag) {
      ...
      if (tag === "all") {
        ...
        const folderKeys = Object.keys(imageUrls).filter(
          (key) => key !== "0_preview"
        );
    ...
    function updateColumns() {
      ...
      // 根据原始顺序排序图片，并去除重复
      const uniqueImages = loadedImages
        .filter((img) => {
      ...
      console.log(
        `调整布局: 共有 ${loadedImages.length} 张图片，去重后 ${uniqueImages.length} 张`
      );
    ...
    function checkIfMoreImagesNeeded() {
      ...
      const contentHeight = Math.max(
        ...columnElements.map((col) => col.offsetHeight || 0)
      );
      ...
      console.log(
        `检查是否需要加载更多: 内容高度=${contentHeight}, 所需高度=${requiredHeight}`
      );
    ...
    function loadNextImages(tag) {
      ...
      // 计算还需要加载多少图片
      let remainingToLoad = Math.max(
        imagesPerRow,
        maxImagesToLoad - loadedImagesCount
      );

      console.log(
        `准备加载图片: 当前已加载=${loadedImagesCount}, 目标总数=${maxImagesToLoad}, 还需加载=${remainingToLoad}`
      );
      ...
      function loadSingleImage(index) {
        ...
        if (
          ...
          document.querySelector(`.gallery img[data-original="${originalUrl}"]`)
        ) {
        ...
        img.onload = function () {
          if (tag !== currentTag) {
            console.log(
              `tag已经切换：当前选中tag:${currentTag}，本请求tag:${tag}，跳过加载`
            );
            return;
          }
          ...
          try {
            ...
            if (
              document.querySelector(
                `.gallery img[data-original="${originalUrl}"]`
              )
            ) {
    ...
    window.addEventListener("scroll", () => {
      ...
      scrollThrottleTimer = setTimeout(() => {
        ...
        if (
          ...
          if (
            currentIndex < (imageUrls[currentTag] || []).length &&
            !isScrollLoading
          ) {
            console.log(
              `触发滚动加载: 滚动位置=${scrollPosition}, 文档高度=${documentHeight}, 内容高度=${contentHeight}`
            );
            isScrollLoading = true;
            loadNextImages(currentTag);
          }
    ...
    function preloadNextBatchImages() {
      ...
      for (let i = currentIndex; i < endIndex; i++) {
        ...
        if (thumbnailUrl.startsWith("/thumbnail/")) {
          ...
          console.log(
            `预加载图片路径转换: ${imageData.thumbnail} -> ${thumbnailUrl}`
          );
        }
      ...
      console.log(
        `预加载了${endIndex - currentIndex}张图片，相当于${Math.ceil(
          (endIndex - currentIndex) / imagesPerRow
        )}行`
      );
    }
          ...
          .then((data) => {
            if (!exifController.signal.aborted) {
              try {
                ...
                exifInfo.innerHTML = `
                                <p>光圈: ${
                                  data.FNumber ? `f/${data.FNumber}` : "N/A"
                                }  ·  快门: ${shutterDisplay}  ·  ISO: ${
                  data.ISO ? data.ISO : "N/A"
                }</p>
                            `;
              } catch (error) {
